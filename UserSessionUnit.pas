...
uses
  IWUserSessionBase, SysUtils, Classes, SimpleGraph, DB, ZAbstractRODataset,
...
type
  ...
  public
    { Public declarations }
    // Variáveis para matrix de dominância problemática de ordenação
    update, updateHord, updateexp, updateC1: boolean;
    SelectedIndex, SelectedIndexHord : array of integer;
    //Fim Yara
    ...
    MatCostPortfolioCB: array of array of Double; // matriz que armazenará vetor cost e custos acuulados exibidos no grid final dos resultados CB
    ...
    MatResultOrd, MatResultOrd1: Array of array of string;
    ...
    qp_ret_exp, vet_poa_ciclo, vet_preferences_ciclo, vet_idA_ciclo,
      vet_idB_ciclo, vet_real_consA_ciclo, holistica, Holistica1: array of string;
    ...
    //// Variável booleana responsável por verificar se a última problemática foi de portfólio
    ///  Entrada Manual para excluir certo a coluna de custos
    antport: boolean;

    /// Verifica o link da análise de sensibilidade na tela do diagrama de Hasse
    lnkAS, passouaqui, Achousol: boolean;
    ...
    primeiro, segundo, veto : boolean; //verifica se apenas um limite foi preenchido

    /// Verifica se a intracritério foi resetada na tela de resultados parciais
    resetintra: boolean;

    /// Verifica se a PLordenação foi rodada duas vezes, ou seja,
    /// A primeira considerando o limiar de 0 e a segunda com base no ep
    verfpl : boolean;

    /// Armazena o valor da variável "Valorep" foi utilizado na PLordenação para encontrar uma solução
    /// Esse armazenamento é importante para saber qual o valore será utilizado
    /// na PLOAS que corresponde a PL da Análise de sensibilidade
    epsol: real;
    ...
    //PARA CLASSIFICAÇÃO (YARA)
    indicesC1: array of Integer; // Array para armazenar os índices [ind] das alternativas em C1 no grdResult1
    countC1: Integer; // Contador para o número de ocorrências de 'C1'
    critIndex: Integer;
    nomealtC1: array of string;
...
////////////////////// variáveis elicitação intracritério- 16.02.22 /////////////////
    ...
    NumDiscNel: integer; // numero de critérios discretos que não são elicitadps
  ...
  //VerificaLin: integer; //verificação se link que declara linear, na intracriterio, foi apertado
   ...
   PortfolioG : Boolean; //** Indica se o sistema está realizando combinações de projetos **//
    ...
    //WAGNER - ALTERAÇÕES INTRA
    //VARIÁVEIS PARA SALVAR OS CICLOS DE RESPOSTAS DA ETAPA INTRACRITÉRIO
    MatrizInfoElicited: array of array of string;  // Matriz que armazena todas as informações elicitadas na etapa intra
    infoCYCLE: string; // Var que guarda todos os ciclos de crit elicitados pra salvar no bd (Junto com a "MatrizInfoElicited")
    Nciclos: integer; // Var que conta a quantidade de ciclos que já foram elicitados
    guardaciclos: array of integer; //Var que irá guardar os ciclos para colocar na exportação

    //VARIÁVEL AUXILIARES PARA PASSAR OS DADOS DAS MATRIZES PARA O BANCO DE DAODS
    infoConsNorm: string; //Var auxiliar que passa os dados da Matriz "ConsNorm" para o Banco de dados
    infoMatrizPontosReais: string; //Var auxiliar que passa os dados da "Matrizpontosreais" (Pontos da Função Valor) para o Banco de dados
    infoCritElicitados: string; //Var auxiliar que indica quais critérios foram elicitados e passa para o Banco de dados (-1 indica que foi elicitado)
    infoPercent: string; //Var auxiliar que atualiza a coluna "percent" do Banco de dados

    //VARIÁVEL PARA AUXILIAR NO RESET DA ELICITAÇÃO
    linhaselecionada: integer;

    // PARA CB (MARIA ELVIRA)
    MatAHPortAux: Array of array of integer;   // Alteração Elvira
    SelectionMatrix: array of array of string; // Alteração Elvira
    MatA_auxport: Array of Array of Double;
    budgetresaux: double; // Alteração elvira
     //**Vetor Auxiliar com os labels dos subgrupos **//
    SubgruposComb, projscbbx : Array of string;
    //** Matriz de consequências normalizadas dos portfólios na rodada atual **//
    matriz_combportNorm: array of array of Double;
    //** Matriz de consequências normalizadas dos portfólios não dominados na rodada atual **//
    matriz_finalNorm: array of array of Double;
    //** Matriz de dominância dos portfólios não dominados na rodada atual **//
    PortfolioDominance : array of array of integer;
    //** Armazena o número de níveis no ranking antes da informação adicionada **//
    RlevelsAnt : integer;
    nvlint : integer;
    BudResult, nproj : Double;
    existimcomp: boolean;
    ele_comuns, eledif1, eledif2, selectedOption, auxsubgruposcomp: string;
    ...
    Position, PositionA1: Integer; // Indica a posição pleiteada pelos critérios
    ...
    // MUDANÇAS AS PORTFOLIO - PEDRO
    auxnport: array of integer;
    //numprojAS: integer;
    vetportfoliofinalAS: array of string; // vetor que armazena as strings dos projetos do portfolio gerado durante a instância da AS
    vetcumulativecostAS: array of Double; // vetor que armazena o custo acumulado dos projetos do portfolio gerado durante a instância da AS
    //coefmatrizgrupoAS: array of array of integer; // matriz que armazena os coeficientes dos projetos de cada portfolio diferente gerado durante a AS

    auxsubset_preferivel, auxsubset_menospreferivel, auxprojetosavaliados, auxdecisaoport: STRING; // salvar ah portcb no BD

     //** Armazena os índices dos elementos comuns a dois portfólios selecionados **//
    vetprojaux: array of integer;

    // ** Variável para definição do número de casas decimais **//
    StrDecimal: array of string;
    StrDInd: Integer;
    StrDIndA, StrDIndB, StrDIndA1, StrDIndB1, StrDIndA2: string;
    ...
    /// ///////// Variáveis utilizadas na Análise de Sensibilidade //////////////
    numproj, numportAS : integer;
    //numproj: armazena o numero de projetos da recomendação do portfólio gerado em cada instância da AS
    //numportAS: armazena o numero de portfólios diferentes gerados durante a AS;
    coefvetorgrupo,coefvetorgrupoAS, numprojAS : array of integer;
    //coefvetorgrupo: vetor que armazena os projetos da recomendação do portfólio;
    //coefvetorgrupoAS: vetor que armazena os projetos da recomendação do portfólio gerado em cada instância da AS
    //numprojAS: vetor que armazena o numero de projetos da recomendação do portfólio gerado em cada instância da AS
    coefmatrizgrupoAS : array of array of integer;
    contnst : integer; // auxiliar que contém o numero de instâncias, utilizado durante a PLCBAS;
    //auxnport : array of integer; // vetor que armazena o numero de vezes que cada portifolio diferente foi recomendado durante a análise de sensibilidade//
    CumulativeCostAS : double; //Armazena o custo total do portifolio gerado pela instancia atual da análise de sensibilidade//
    PortfoliofinalAS : string; //Armazena os projetos do portifolio gerado pela instancia atual da análise de sensibilidade//
    BudgetAS, CriAS, AltAS : boolean; // Variaveis booleanas que indicam o tipo de analise de senbilidade para Portfólio a ser realizada: "CriAS" para a AS das consequencias; "AltAS" para a AS dos custos das alternativas; "BudgetAS" para a AS do budget.
    CostNormAS, CostAS: Array of Double; //CostAS: Vetor que armazena o custo das alternativas durante a AS dos custos; CostNormAS: Vetor que armazena o custo normalizado das alternativas durante a AS dos custos
    MinCostAS, MaxCostAS, FmultAS : Double;
    // MinCostAS: Limite inferior do custo da alternativa para a AS dos custos para a problemática de Portfólio;
    // MaxCostAS: Limite superior do custo da alternativa para a AS dos custos para a problemática de Portfólio;
    //FmultAS : Fator multiplicativo utilizado na AS para Portfólio;
    /// ////////////////////////////////////////////////////////////////////////
    VauxSel2: integer; // Armazena o índice do critério selecionado
    pmax2, pmin2: array of Double; // Amazena os percentuais de variáção
    budgetvariation, ASBudget : double;
    altsele: array of boolean; // crisel: Indica se o critério foi ou não selecionado; altsel: Indica se a alternativa foi ou não selecionada
    edtmaxAS : array of TiwEdit;
    edtminAS : array of TiwEdit;
    edtmaxASProjects : array of TiwEdit;
    edtminASProjects : array of TiwEdit;
    edtpercentprofilesmax, edtpercentprofilesmin : array of TiwEdit;
    cmbbxmaxAS, cmbbxminAS : array of Tiwcombobox;
    lblmax, lblmin, lblmaxprofiles, lblminprofiles, lblmaxprojects, lblminprojects : array of Tiwlabel;
    lblvariationmax, lblvariationmin, lblvariationmaxprofiles, lblvariationminprofiles, lblvariationmaxprojects, lblvariationminprojects : array of tiwlabel;
   ...
   //TESTE DE KENDALL - PEDRO
    ...
    Pmean : double; // Mudança teste de Kendall - Pedro 23/04/2025
    ordemalpha : array of integer; // Mudança teste de Kendall - Pedro 23/04/2025
    freqalpha : array of integer;
    ////////////////////////////////
     // PROPOSTA AS - PEDRO
    Incrementa, Incrementa2 : integer;
    reiniciaAS : integer;
    auxgridAS : TiwGrid;
    tbsheet : array of TIWTabPage;
    auxlblas : array of TiwLabel;
    auxregionAS : TiwRegion;
    qtdranking : array of integer;
    matrizposicao : array of array of array of integer;
    AltSel : array of boolean;
    perfisAS : array of real;
    MatrizDominanciaAS : array of array of integer;

    //Tela do Analista
    heuristica: boolean;

    ///Variavel booleana responsavel pela verificação se o botão de exportação em HTML foi clicado ou não
    html: boolean;
    kendall: boolean;//conferindo se o teste de Kendall foi realiado

    //Problema na retomada quando não tem mais perguntas na decomposição e o problema não foi finalizado
    solint : string;  //Salva quando a solução é = 10
    ...
    ResultadoAlpha : array [0..101] of integer;

    //Sequencia de visualização
    Resp, vis, seq_cam: string;
    RespPC, visPC, seq_camPC, verbd, ultimovalor: string;

 end;

implementation

uses ServerController, uResultados;
